...
#let template(
  ...
  // 设置数学公式中引用中文的字体
  show math.text: set text(font: (fonts.en-font, fonts.zh-font))
  ...
  if ifcontest == true {
    ...
    show outline.entry.where(level: 1): entry => {
      ...
      //  组装：“第一章 标题 …… 页码”且整行加粗
      link(loc, entry.indented(
        strong("第" + cn(chap) + "章"),
        strong(entry.body()) + h(1fr) + strong(entry.page()),
      ))
    }

    // 3) 去掉一级条目的 leader dots
    // set outline.entry.where(level: 1)(fill: none)
...
#let problem = thmenv("problem", boxcounting, 2, (name, number, body) => {
  notebox(name, number, body, "问题", probSvg, blue)
}).with(numbering: boxnumbering)

#let answer = thmenv("answer", boxcounting, 2, (name, number, body) => {
  notebox(name, number, body, "答案", ansSvg, olive)
}).with(numbering: boxnumbering)
